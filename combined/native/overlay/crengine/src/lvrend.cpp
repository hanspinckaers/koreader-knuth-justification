...
//                }
                ...
                for (int i=0; i<count; i++) {
                    ...
                    context.AddLine(rect.top + line->y + padding_top,
                        rect.top + line->y + line->height + padding_top,
                        line_flags, line);
...
public:
    ...
    void addContentLine( int height, int flags, int baseline=NO_BASELINE_UPDATE,
        ...
        if ( !(flags & RN_SPLIT_BEFORE_AVOID) && hasFloatRunningAtY(c_y) )
            flags |= RN_SPLIT_BEFORE_AVOID;
        flags |= line_dir_flag;
        context.AddLine( c_y, c_y + height, flags, formatted_line );
        last_split_after_flag = RN_GET_SPLIT_AFTER(flags);
        if ( !is_padding )
