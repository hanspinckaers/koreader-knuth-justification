...
int LVDocView::getPageVerticalOffset(const LVRendPageInfo & page, const lvRect & pageRect) {
	if (!m_props->getBoolDef(PROP_PAGE_CENTER_NEARLY_FULL, true)
			|| getViewMode() != DVM_PAGES
			|| !(page.flags & RN_PAGE_TYPE_NORMAL)
			|| (page.flags & RN_PAGE_TYPE_COVER)
			|| page.height <= 0
			|| page.footnotes.length() > 0) {
		return 0;
	}
	int available_height = pageRect.height() - m_pageMargins.top
			- m_pageMargins.bottom - getPageHeaderHeight();
	int unused_height = available_height - page.height;
	if (unused_height <= 0)
		return 0;
	int line_height = m_font.isNull() ? m_font_size : m_font->getHeight();
	line_height = line_height * m_def_interline_space / 100;
	if (line_height < 1)
		line_height = 1;
	// Only balance the small remainder left after fitting a whole number of
	// lines. Short chapter endings and title pages stay top-aligned.
	if (unused_height > line_height)
		return 0;
	return unused_height / 2;
}
...
void LVDocView::drawPageTo(LVDrawBuf * drawbuf, LVRendPageInfo & page,
	...
	if (!pageRect)
		pageRect = &fullRect;
	int vertical_offset = getPageVerticalOffset(page, *pageRect);
	...
	lvRect clip;
	clip.top = pageRect->top + m_pageMargins.top + headerHeight + vertical_offset;
	clip.bottom = pageRect->top + m_pageMargins.top + height + headerHeight + vertical_offset;
...
bool LVDocView::windowToDocPoint(lvPoint & pt, bool pullInPageArea) {
	...
	if (getViewMode() == DVM_SCROLL) {
		...
		int headerHeight = getPageHeaderHeight();
		int page1_vertical_offset = page >= 0 && page < m_pages.length()
				? getPageVerticalOffset(*m_pages[page], m_pageRects[0]) : 0;
		int page_vertical_offset = page1_vertical_offset;
		...
		if ( pullInPageArea ) {
			if ( getVisiblePageCount() < 2 || pt.x <= m_dx / 2) {
				...
				if ( pt.x >= page1.right )
					pt.x = page1.right - 1;
				if (page >= 0 && page < m_pages.length() && m_pages[page]->height > 0) {
					int content_top = page1.top + page1_vertical_offset;
					int content_bottom = content_top + m_pages[page]->height;
					if ( pt.y < content_top )
						pt.y = content_top;
					if ( pt.y >= content_bottom )
						pt.y = content_bottom - 1;
				}
			}
		}
		lvRect page2;
		int page2_vertical_offset = 0;
		if (page1.isPointInside(pt)) {
			rc = &page1;
		} else if (getVisiblePageCount() == 2) {
			page2 = m_pageRects[1];
			bool page2_exists = page + 1 >= 0 && page + 1 < m_pages.length();
			if (page2_exists)
				page2_vertical_offset = getPageVerticalOffset(*m_pages[page + 1], m_pageRects[1]);
			...
			if ( pullInPageArea ) {
				...
				if ( pt.x >= page2.right )
					pt.x = page2.right - 1;
				if (page2_exists && m_pages[page + 1]->height > 0) {
					int content_top = page2.top + page2_vertical_offset;
					int content_bottom = content_top + m_pages[page + 1]->height;
					if ( pt.y < content_top )
						pt.y = content_top;
					if ( pt.y >= content_bottom )
						pt.y = content_bottom - 1;
			...
			if (page2_exists && page2.isPointInside(pt)) {
				rc = &page2;
				page++;
				page_vertical_offset = page2_vertical_offset;
			}
		}
		if (rc && page >= 0 && page < m_pages.length()) {
			int page_y = m_pages[page]->start;
			pt.x -= rc->left;
			pt.y -= rc->top + page_vertical_offset;
			if ( pullInPageArea ) {
				if ( pt.y >= m_pages[page]->height )
					pt.y = m_pages[page]->height - 1;
			}
			if (pt.y >= 0 && pt.y < m_pages[page]->height) {
            ...
            if (page >= 0 && page < m_pages.length() && pt.y >= m_pages[page]->start) {
                ...
                if (index >= 0) {
                    ...
                    */
                    ...
                    int vertical_offset = getPageVerticalOffset(
                            *m_pages[page + index], m_pageRects[index]);
                    pt.y = pt.y + getPageHeaderHeight() + m_pageMargins.top
                            + vertical_offset - m_pages[page + index]->start;
                    return true;
                }
            }
            if (fitToPage) {
                ...
                if (page >= 0 && page < m_pages.length() && pt.y < m_pages[page]->start) {
                    ...
                    pt.y = getPageHeaderHeight() + m_pageMargins.top
                            + getPageVerticalOffset(*m_pages[page], m_pageRects[0]);
                }
                else if (getVisiblePageCount() == 2 && page + 1 < m_pages.length()
                    ...
                    pt.y = getPageHeaderHeight() + m_pageMargins.top
                            + getPageVerticalOffset(*m_pages[page + 1], m_pageRects[1])
                            + m_pages[page+1]->height;
                }
                else {
...
//	props->setIntDef(PROP_TXT_OPTION_PREFORMATTED, 0);
	...
	props->limitValueList(PROP_PAGE_CENTER_NEARLY_FULL, bool_options_def_true, 2);
    ...
    for (int i = 0; i < props->getCount(); i++) {
        ...
        //     REQUEST_RENDER("propsApply - kerning")
            ...
            if (name == PROP_PAGE_MARGIN_TOP)
                ...
                rc.right = margin;
            setPageMargins(rc);
        } else if (name == PROP_PAGE_CENTER_NEARLY_FULL) {
            bool old_value = m_props->getBoolDef(PROP_PAGE_CENTER_NEARLY_FULL, true);
            bool new_value = props->getBoolDef(PROP_PAGE_CENTER_NEARLY_FULL, true);
