...
void LVRendPageContext::AddLine( int starty, int endy, int flags,
                                const formatted_line_t * formatted_line )
{
    ...
    LVRendLineInfo * line = new LVRendLineInfo(
            starty, endy, flags, current_flow, formatted_line);
    lines.add( line );
    current_flow_empty = false;
    if ( curr_note != NULL ) {
...
void LVRendPageContext::split()
{
    ...
    // Derive one visible word-space baseline from every justified body line
    // that the page splitter actually placed on a page. The formatter's line
    // metric already includes left/right hanging punctuation, so this target
    // represents the optical page measure rather than the raw glyph advance.
    int page_count = page_list->length();
    std::vector<KnuthPageSpacingAccumulator> page_spacing(page_count);
    for ( int i=0; i<page_count; i++ )
        page_list->get(i)->word_spacing_target_x64 = -1;

    for ( int i=0; i<lines.length(); i++ ) {
        LVRendLineInfo * rend_line = lines[i];
        const formatted_line_t * line = rend_line->getFormattedLine();
        if ( !line || line->justify_space_count <= 0 ||
                line->justify_space_target_x64 <= 0 ||
                (rend_line->getFlags() & RN_SPLIT_FOOT_NOTE) )
            continue;

        int page_index = -1;
        for ( int p=0; p<page_count; p++ ) {
            LVRendPageInfo * page = page_list->get(p);
            if ( !(page->flags & RN_PAGE_TYPE_NORMAL) ||
                    page->flow != rend_line->flow )
                continue;
            if ( rend_line->getStart() >= page->start &&
                    rend_line->getStart() < page->start + page->height ) {
                page_index = p;
                break;
            }
        }
        if ( page_index < 0 )
            continue;
        page_spacing[page_index].add(line->justify_space_target_x64,
                                     line->justify_space_count);
    }
    for ( int i=0; i<page_count; i++ ) {
        page_list->get(i)->word_spacing_target_x64 =
                (lInt32)page_spacing[i].targetX64();
    }
}
...
static const char * pagelist_magic = "PageListKnuth2";
...
bool LVRendPageInfo::serialize( SerialBuf & buf )
{
    ...
    buf << (lInt32)word_spacing_target_x64;
...
bool LVRendPageInfo::deserialize( SerialBuf & buf )
{
    ...
    buf >> n1 >> n2 >> n5 >> n3 >> n4; /// start of page

    start = n1;
    height = n2;
    word_spacing_target_x64 = n5;
    flags = n3;
    flow = n4;
