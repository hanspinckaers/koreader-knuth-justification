...
public:
    ...
    lInt32 word_spacing_target_x64; /// page-wide visible word gap, or -1
    ...
    : start(pageStart), index(pageIndex), height(pageHeight),
      word_spacing_target_x64(-1), flags(RN_PAGE_TYPE_NORMAL), flow(0) {}
    LVRendPageInfo(lUInt16 coverHeight)
    : start(0), index(0), height(coverHeight), word_spacing_target_x64(-1),
      flags(RN_PAGE_TYPE_COVER), flow(0) {}
    LVRendPageInfo()
    : start(0), index(0), height(0), word_spacing_target_x64(-1),
      flags(RN_PAGE_TYPE_NORMAL), flow(0) {}
...
class LVRendLineInfo {
    ...
    const formatted_line_t * formatted_line; // optional Knuth page metric
    int start;              // 4 bytes
...
*/
    ...
    LVRendLineInfo() : links(NULL), formatted_line(NULL), start(-1), height(0), flags(0), flow(0) { }
    LVRendLineInfo( int line_start, int line_end, lUInt16 line_flags )
    : links(NULL), formatted_line(NULL), start(line_start), height(line_end-line_start), flags(line_flags), flow(0)
    {
    }
    LVRendLineInfo( int line_start, int line_end, lUInt16 line_flags, int flow,
                    const formatted_line_t * source_line=NULL )
    : links(NULL), formatted_line(source_line), start(line_start),
      height(line_end-line_start), flags(line_flags), flow(flow)
    {
    }
    LVFootNoteList * getLinks() { return links; }
    const formatted_line_t * getFormattedLine() const { return formatted_line; }
    ~LVRendLineInfo()
    {
        clear();
    }
...
public:
    ...
    void AddLine( int starty, int endy, int flags,
                  const formatted_line_t * formatted_line=NULL );
