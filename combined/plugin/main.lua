...
function KnuthJustification:applyLiterataComparisonPreset(page_level)
    self.ui:handleEvent(Event:new("SetFont", "Literata"))
    self.ui:handleEvent(Event:new("SetFontSize", 16))
    self.ui:handleEvent(Event:new("SetFontKerning", 3))
    self.ui:handleEvent(Event:new("SetFontFractionalPositioning", 3))
    self.ui:handleEvent(Event:new("ToggleStyleTweak",
        { "text_align_most_justify", true }))
    self.ui:handleEvent(Event:new("ToggleFloatingPunctuation", true))

    if self.ui.typography then
        self.ui.typography.hyphenation = true
    end
    self.ui.document:setTextHyphenation(true)
    self.ui.doc_settings:saveSetting("hyphenation", true)

    self:setValue("page_center_nearly_full", 1, true)
    self:setValue("justification_word_spacing", { 25, 35 }, true)
    self:setValue("justification_letter_spacing", { 1, 0 }, true)
    self:setValue("justification_tracking_smoothness", 50, true)
    self:setValue("justification_tolerance", { 100, 200 }, true)
    self:setValue("justification_line_penalty", { 10, 10000 }, true)
    self:setValue("justification_line_limits", { 6, 33 }, true)
    self:setValue("line_breaking_mode", page_level and 1 or 0, true)

    Notification:notify(page_level
        and _("Applied Literata 16 pt · Best/High · Page-level")
        or _("Applied Literata 16 pt · Best/High · Greedy"))
    return true
end
...
function KnuthJustification:getMenuTable()
    local menu = {
        {
            text = _("Literata 16 pt · Best/High · Greedy"),
            callback = function()
                self:applyLiterataComparisonPreset(false)
            end,
        },
        {
            text = _("Literata 16 pt · Best/High · Page-level"),
            callback = function()
                self:applyLiterataComparisonPreset(true)
            end,
            separator = true,
        },
        {
