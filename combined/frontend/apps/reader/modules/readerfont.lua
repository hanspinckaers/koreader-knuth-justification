...
local function normalizeKerningPrecision(value)
    if type(value) == "boolean" then
        return value and 3 or 0
    end
...
function ReaderFont:onReadSettings(config)
    ...
    self.configurable.font_fractional_positioning = normalizeKerningPrecision(
        self.configurable.font_fractional_positioning)
...
function ReaderFont:onSetFontFractionalPositioning(strength)
    strength = normalizeKerningPrecision(strength)
    self.configurable.font_fractional_positioning = strength
    self.ui.document:setFontFractionalPositioning(strength)
    self.ui:handleEvent(Event:new("UpdatePos"))
    Notification:notify(T(_("Kerning precision set to: %1"),
        optionsutil:getOptionText("SetFontFractionalPositioning", strength)))
    return true
end
